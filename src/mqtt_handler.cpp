            ...
            if (thumbnailPendingPublish) {
                ...
                if (display.getBuffer() != nullptr) {
                    ...
                    if (jsonFromSD != nullptr) {
                        Serial.println("Loaded thumbnail from SD card, encrypting and publishing...");
                        // Encrypt the thumbnail JSON before publishing (consistent with publishMQTTThumbnail)
                        String plaintextJson = String(jsonFromSD);
                        free(jsonFromSD);
                        jsonFromSD = nullptr;
                        
                        String encryptedJson = encryptAndFormatMessage(plaintextJson);
                        if (encryptedJson.length() == 0) {
                            Serial.println("ERROR: Failed to encrypt SD thumbnail - skipping publish");
                        } else {
                            int msg_id = esp_mqtt_client_publish(client, mqttTopicThumb, encryptedJson.c_str(), encryptedJson.length(), 1, 1);
                            if (msg_id > 0) {
                                bool isEncrypted = isEncryptionEnabled();
                                Serial.printf("Published %s thumbnail from SD to %s (msg_id: %d)\n", 
