...
<thead><tr style="background:#333;"><th style="padding:10px;text-align:left;width:20%;">Image</th><th style="padding:10px;text-align:left;width:20%;">Audio</th><th style="padding:10px;text-align:left;width:12%;">Foreground</th><th style="padding:10px;text-align:left;width:12%;">Outline</th><th style="padding:10px;text-align:left;width:12%;">Font</th><th style="padding:10px;text-align:left;width:10%;">Thickness</th><th style="padding:10px;text-align:center;width:8%;">Next</th><th style="padding:10px;text-align:center;width:8%;">Actions</th><th style="padding:10px;width:8%;"></th></tr></thead>
...
function renderFileList(){const list=document.getElementById('fileList');if(fileListData.length===0){list.innerHTML='<p>No files found'+(currentDir?' in '+currentDir:'')+'.</p>';return;}const sorted=fileListData.slice().sort((a,b)=>{let valA,valB;if(fileListSortCol==='name'){valA=a.name.toLowerCase();valB=b.name.toLowerCase();}else if(fileListSortCol==='size'){valA=a.size;valB=b.size;}else if(fileListSortCol==='modified'){valA=a.modified||0;valB=b.modified||0;}return valA<valB?-1*fileListSortDir:valA>valB?1*fileListSortDir:0;});let html='';if(currentDir){const parts=currentDir.split('/').filter(p=>p.length>0);html+='<div style="margin-bottom:10px;padding:8px;background:#333;border-radius:4px;"><span style="cursor:pointer;color:#4CAF50;" onclick="navigateToDir(\'\')">Root</span>';parts.forEach((part,idx)=>{const path=parts.slice(0,idx+1).join('/');html+=` / <span style="cursor:pointer;color:#4CAF50;" onclick="navigateToDir('${path}')">${escapeHtml(part)}</span>`;});html+='</div>';}html+='<table style="width:100%;border-collapse:collapse;"><thead><tr style="background:#333;">';html+=`<th style="padding:8px;text-align:left;cursor:pointer;" onclick="sortFileList('name')">Name ${fileListSortCol==='name'?(fileListSortDir>0?'▲':'▼'):''}</th>`;html+=`<th style="padding:8px;text-align:right;cursor:pointer;" onclick="sortFileList('size')">Size ${fileListSortCol==='size'?(fileListSortDir>0?'▲':'▼'):''}</th>`;html+=`<th style="padding:8px;text-align:left;cursor:pointer;" onclick="sortFileList('modified')">Last Modified ${fileListSortCol==='modified'?(fileListSortDir>0?'▲':'▼'):''}</th>`;html+='<th style="padding:8px;text-align:center;width:200px;">Actions</th></tr></thead><tbody>';sorted.forEach(f=>{const size=f.isDir?'&lt;DIR&gt;':f.size>=1024*1024?(f.size/(1024*1024)).toFixed(2)+' MB':f.size>=1024?(f.size/1024).toFixed(2)+' KB':f.size+' B';const modified=formatDate(f.modified);const nameEscaped=escapeJs(f.name);const pathEscaped=escapeJs(f.path);const nameHtml=escapeHtml(f.name);if(f.isDir){html+=`<tr><td style="padding:8px;"><span style="cursor:pointer;color:#4CAF50;font-weight:bold;" onclick="navigateToDir('${pathEscaped}')">📁 ${nameHtml}</span></td><td style="padding:8px;text-align:right;">${size}</td><td style="padding:8px;">${modified}</td><td style="padding:8px;text-align:center;"><button onclick="renameFile('${pathEscaped}','${nameEscaped}')" style="margin:2px;padding:4px 8px;font-size:12px;">Rename</button><button onclick="deleteFile('${pathEscaped}')" class="delete" title="Delete">✕</button></td></tr>`;}else{html+=`<tr><td style="padding:8px;">${nameHtml}</td><td style="padding:8px;text-align:right;">${size}</td><td style="padding:8px;">${modified}</td><td style="padding:8px;text-align:center;"><button onclick="downloadFile('${pathEscaped}')" style="margin:2px;padding:4px 8px;font-size:12px;">Download</button><button onclick="renameFile('${pathEscaped}','${nameEscaped}')" style="margin:2px;padding:4px 8px;font-size:12px;">Rename</button><button onclick="deleteFile('${pathEscaped}')" class="delete" title="Delete">✕</button></td></tr>`;}});html+='</tbody></table>';list.innerHTML=html;}
...
let canvas=null;let ctx=null;let isDrawing=false;let lastX=0;let lastY=0;
const colorMap={0:'#000000',1:'#FFFFFF',2:'#FFFF00',3:'#FF0000',5:'#0000FF',6:'#00FF00'};
function getDrawColor(){const drawColorEl=document.getElementById('drawColor');if(!drawColorEl)return'#000000';const val=parseInt(drawColorEl.value);return colorMap[val]||'#000000';}
function startDraw(e){if(!canvas)return;isDrawing=true;const rect=canvas.getBoundingClientRect();lastX=e.clientX?e.clientX-rect.left:e.touches[0].clientX-rect.left;lastY=e.clientY?e.clientY-rect.top:e.touches[0].clientY-rect.top;}
function draw(e){if(!isDrawing||!canvas||!ctx)return;e.preventDefault();const rect=canvas.getBoundingClientRect();const x=e.clientX?e.clientX-rect.left:e.touches[0].clientX-rect.left;const y=e.clientY?e.clientY-rect.top:e.touches[0].clientY-rect.top;ctx.strokeStyle=getDrawColor();ctx.lineWidth=2;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(lastX,lastY);ctx.lineTo(x,y);ctx.stroke();lastX=x;lastY=y;}
function stopDraw(){isDrawing=false;}
function clearCanvas(){if(!ctx)return;ctx.fillStyle='#FFFFFF';ctx.fillRect(0,0,800,600);showStatus('canvasStatus','Canvas cleared',false);}
...
const size_t WEB_HTML_CONTENT_LEN = 47957;
