...
/**
 * Handle canvas_display command - processes large canvas data without full JSON parsing
 * Extracted from handleWebInterfaceCommand to reduce complexity
 */
...
/**
 * Save canvas pixel data as PNG to SD card
 * Converts e-ink color indices to RGB and encodes as PNG
 */
bool saveCanvasAsPNG(const uint8_t* pixelData, size_t pixelCount, unsigned width, unsigned height, const String& filename) {
    ...
    size_t pngSize = 0;
    unsigned error = lodepng_encode24(&pngData, &pngSize, rgbData, width, height);
