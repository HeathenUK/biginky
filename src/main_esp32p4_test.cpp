...
static bool mqttMessageRetain = false;  // Store retain flag from first chunk
static char mqttMessageTopic[128] = "";  // Store topic from first chunk
    ...
    switch (event->event_id) {
        ...
        case MQTT_EVENT_DATA: {
            ...
            if (event->current_data_offset == 0) {
                ...
                mqttMessageBufferUsed = 0;
                mqttMessageRetain = event->retain;  // Store retain flag from first chunk
                
                // Store topic from first chunk
            ...
            Serial.printf("Message details: stored_topic='%s', current_event_topic='%s', stored_retain=%d, current_event_retain=%d, first_char='%c' (0x%02x)\n", 
                         mqttMessageTopic, topic, mqttMessageRetain ? 1 : 0, event->retain ? 1 : 0,
            ...
            Serial.printf("Topic comparison: stored='%s', current='%s', expected_webui='%s', stored_match=%d, current_match=%d\n",
                         mqttMessageTopic, topic, mqttTopicWebUI, 
                         strcmp(mqttMessageTopic, mqttTopicWebUI) == 0 ? 1 : 0,
...
static bool handleWebInterfaceCommand(const String& jsonMessage) {
    ...
    // canvas_display is handled above (before JSON parsing) to avoid memory issues with 640KB messages
