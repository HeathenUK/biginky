...
// SD card support for keep-out map loading (RP2350 only - uses SdFat)
// ESP32-P4 uses FatFs and doesn't need this include
...
/**
 * @brief Keep-out map loaded from SD card (ML-generated object masks)
 * 
 * Maps are binary bitmaps marking areas where text should not be placed,
 * typically generated by ML object detection to avoid overlapping with
 * salient image content.
 */
struct KeepOutMap {
    uint16_t width;       ///< Map width (should match display width)
    uint16_t height;      ///< Map height (should match display height)
    uint8_t* bitmap;      ///< Packed bitmap (1 bit per pixel, 1=keep out)
    
    KeepOutMap() : width(0), height(0), bitmap(nullptr) {}
    ...
    /// Check if a specific pixel is in a keep-out area
    bool isKeepOut(int16_t x, int16_t y) const {
        if (!bitmap || x < 0 || y < 0 || x >= width || y >= height) {
            return false;
        }
        
        uint32_t pixelIdx = (uint32_t)y * width + x;
        uint32_t byteIdx = pixelIdx / 8;
        uint8_t bitIdx = 7 - (pixelIdx % 8);  // MSB first
        
        return (bitmap[byteIdx] & (1 << bitIdx)) != 0;
    }
    
    /// Check if any pixels in a rectangle overlap keep-out areas
    bool overlapsKeepOut(int16_t x, int16_t y, int16_t w, int16_t h) const {
        if (!bitmap) return false;
        
        // Sample at regular intervals (checking every pixel is expensive)
        // Use 8x8 grid sampling for speed
        int stepX = (w > 16) ? (w / 8) : 1;
        int stepY = (h > 16) ? (h / 8) : 1;
        
        for (int16_t dy = 0; dy < h; dy += stepY) {
            for (int16_t dx = 0; dx < w; dx += stepX) {
                if (isKeepOut(x + dx, y + dy)) {
                    return true;
                }
            }
        }
    ...
    /// Calculate percentage of rectangle that overlaps keep-out areas
    float getKeepOutCoverage(int16_t x, int16_t y, int16_t w, int16_t h) const {
        if (!bitmap || w <= 0 || h <= 0) return 0.0f;
        
        // Sample at regular intervals
        int stepX = (w > 32) ? (w / 16) : 1;
        int stepY = (h > 32) ? (h / 16) : 1;
        
        int totalSamples = 0;
        int keepOutSamples = 0;
        ...
        return (totalSamples > 0) ? ((float)keepOutSamples / totalSamples) : 0.0f;
    }
};
 ...
 * 
 * Supports ML-generated keep-out maps from SD card for object-aware placement.
 */
...
public:
    ...
    // ========================================================================
    // Keep-Out Map - ML-generated object masks
    // ========================================================================
    
    /**
     * @brief Load keep-out map from a file (SD card) - RP2350/SdFat only
     * 
     * Map files are generated by prepare_eink_image.py and contain binary
     * bitmaps marking areas where ML detected objects (1 bit per pixel).
     * 
     * File format:
     *   Header (16 bytes):
     *     - Magic: "KOMAP" (5 bytes)
     ...
     *     - Reserved: 6 bytes
     *   Data:
     *     - Bitmap: (width * height + 7) / 8 bytes (1 bit per pixel, MSB first)
     * 
     * @param file Open file handle positioned at start of map file
    ...
    /**
     * @brief Get the loaded keep-out map (for inspection)
     */
    const KeepOutMap& getKeepOutMap() const { return _keepOutMap; }
...
private:
    ...
    // Keep-out map - ML-generated object masks from SD card
    KeepOutMap _keepOutMap;
