...
bool pngDrawRandomToBuffer(const char* dirname, uint32_t* out_sd_read_ms, uint32_t* out_decode_ms) {
    ...
    // Detailed SD read benchmark output
    float sizeMB = fileSize / (1024.0f * 1024.0f);
    float speedMBs = (loadTime > 0) ? (sizeMB / (loadTime / 1000.0f)) : 0;
    Serial.printf("SD read: %.2f MB in %lu ms = %.2f MB/s", sizeMB, loadTime, speedMBs);
    // Performance expectation note (SRAM buffers should give 8-15 MB/s, PSRAM ~1-2 MB/s)
    if (speedMBs >= 5.0f) {
        Serial.println(" [SRAM buffers working]");
    } else if (speedMBs >= 1.0f) {
        Serial.println(" [possibly PSRAM buffers - check ff_memalloc]");
    } else {
        Serial.println(" [slow - check SD card/connection]");
    }
