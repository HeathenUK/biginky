...
int SIMCom_A7683E::waitResponse(uint32_t timeout_ms, String& data) {
    ...
    // NOTE: +CMGL and +CMGR are intentionally omitted so SMS list/read responses
    // reach the caller.
...
bool SIMCom_A7683E::getSMSCount(int* unread, int* total) {
    ...
    *total = 0;
    ...
    // Set character set and text mode for consistent parsing
    if (!sendAT("AT+CSCS=\"IRA\"")) {
        Serial.println("SIMCom A7683E: getSMSCount - Failed to set CSCS=\"IRA\"");
        return false;
    }
    delay(150);
...
bool SIMCom_A7683E::listSMS(int max_messages) {
    ...
    (void)max_messages;  // Currently unused; retained for compatibility/verbosity control
    ...
    // Determine current storage
    String current_storage = "SM";
    String current_cpms;
    if (sendAT("AT+CPMS?", current_cpms, 3000)) {
        ...
        if (cpms_start >= 0) {
            ...
            if (quote_end > cpms_start) {
                ...
                Serial.printf("SIMCom A7683E: Parsed current storage as: %s\n", current_storage.c_str());
            }
        }
    } else {
        Serial.println("SIMCom A7683E: Unable to query current SMS storage, defaulting to SM");
    }

    auto listFrom = [&](const char* store) -> bool {
        if (!current_storage.equals(store)) {
            Serial.printf("SIMCom A7683E: Switching SMS storage to %s...\n", store);
            String cpms_set_response;
            String cmd = String("AT+CPMS=\"") + store + "\",\"" + store + "\",\"" + store + "\"";
            if (!sendAT(cmd.c_str(), cpms_set_response, 5000)) {
                Serial.printf("SIMCom A7683E: Failed to switch to %s storage. Response: [%s]\n", store, cpms_set_response.c_str());
                return false;
            }
            Serial.printf("SIMCom A7683E: Storage switched to %s. Response: [%s]\n", store, cpms_set_response.c_str());
            delay(200);
        ...
        bool ok = sendAT("AT+CMGL=\"ALL\"", list_response, 30000);
        if (!list_response.isEmpty()) {
            Serial.print(list_response);
        }
        if (!ok) {
            Serial.printf("SIMCom A7683E: AT+CMGL failed for %s storage\n", store);
        }
        return ok;
    };

    if (listFrom(current_storage.c_str())) found_ok = true;
    if (!current_storage.equals("SM") && listFrom("SM")) found_ok = true;
    if (!current_storage.equals("ME") && listFrom("ME")) found_ok = true;
