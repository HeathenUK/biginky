/**
 * @file main.cpp
 * @brief ESP32-P4 application entry point
 *
 * This application drives the EL133UF1 display on the ESP32-P4 platform.
 * It focuses on ESP32-P4 hardware only; RP2350/Pico support has been
 * removed to simplify the project configuration.
 *
 ...
 *
 * Legacy RP2350 notes retained for reference:
 *   - Wake button: GPIO1 (active-low to ground) used for waking from sleep
 *   - SD card detect switch: GPIO37 for card-present sensing
 */
...
#if __has_include("esp_audio_simple_player.h")
#include "esp_audio_simple_player.h"
#include "esp_codec_dev.h"
#include "esp_codec_dev_defaults.h"
#else
#error "esp_audio_simple_player.h is required for WAV playback; ensure esp-gmf is available."
#endif
#else
#include "esp_audio_simple_player.h"
#include "esp_codec_dev.h"
...
static bool audio_i2s_init(uint32_t sample_rate_hz) {
    if (g_i2s_tx != nullptr) {
        if (g_audio_sample_rate_hz == sample_rate_hz) {
            return true;
        }
    ...
    g_i2s_tx = tx_handle;
    g_audio_sample_rate_hz = sample_rate_hz;
    return true;
}
...
static bool audio_start(bool verbose, uint32_t sample_rate = 44100, bool start_tone = true) {
    const int bits = 16;

    if (g_codec_ready && g_audio_sample_rate_hz != sample_rate) {
        audio_stop();
    }

    if (g_audio_running && start_tone && g_audio_sample_rate_hz == sample_rate) {
...
static bool gmf_init_codec_chain(uint32_t preferred_sample_rate) {
    if (g_gmf_codec && g_gmf_player) {
        return true;
    }

    audio_codec_i2s_cfg_t i2s_cfg = AUDIO_CODEC_I2S_CFG_DEFAULT();
    i2s_cfg.role = AUDIO_CODEC_I2S_ROLE_MASTER;
    i2s_cfg.port = I2S_NUM_0;
    i2s_cfg.clk_cfg.sample_rate_hz = preferred_sample_rate;
    i2s_cfg.dma_buf_count = 8;
    i2s_cfg.dma_buf_len = 512;

    const audio_codec_data_if_t* i2s_data_if = audio_codec_new_i2s_data(&i2s_cfg);
    if (!i2s_data_if) {
        Serial.println("esp-gmf: failed to create I2S data interface");
        return false;
    }

    audio_codec_i2c_cfg_t i2c_cfg = AUDIO_CODEC_I2C_CFG_DEFAULT();
    i2c_cfg.addr = PIN_CODEC_I2C_ADDR;
    i2c_cfg.sda = PIN_CODEC_I2C_SDA;
    i2c_cfg.scl = PIN_CODEC_I2C_SCL;
    const audio_codec_ctrl_if_t* i2c_ctrl_if = audio_codec_new_i2c_ctrl(&i2c_cfg);
    if (!i2c_ctrl_if) {
        Serial.println("esp-gmf: failed to create codec I2C control interface");
        return false;
    }

    audio_codec_es8311_cfg_t es_cfg = AUDIO_CODEC_ES8311_DEFAULT_CFG();
    es_cfg.ctrl_if = i2c_ctrl_if;
    es_cfg.master_mode = false;
    es_cfg.use_mclk = true;
    es_cfg.mclk_div = 256;
    es_cfg.pa_pin = PIN_CODEC_PA_EN;
    es_cfg.pa_reverted = false;

    const audio_codec_if_t* es_if = audio_codec_new_es8311(&es_cfg);
    if (!es_if) {
        Serial.println("esp-gmf: failed to create ES8311 codec interface");
        return false;
    }
    ...
    g_gmf_codec = esp_codec_dev_new(&dev_cfg);
    if (!g_gmf_codec) {
        Serial.println("esp-gmf: failed to create codec device");
        return false;
    }

    esp_audio_simple_player_cfg_t player_cfg = ESP_AUDIO_SIMPLE_PLAYER_DEFAULT_CFG();
    player_cfg.task_prio = 5;
    player_cfg.task_stack = 4096;
    player_cfg.prefer_sample_rate = preferred_sample_rate;
    player_cfg.prefer_channel = 2;
    player_cfg.out = g_gmf_codec;

    g_gmf_player = esp_audio_simple_player_create(&player_cfg);
    if (!g_gmf_player) {
        Serial.println("esp-gmf: failed to create simple player");
        return false;
    }

    (void)esp_codec_dev_set_out_vol(g_gmf_codec, g_audio_volume_pct);
    return true;
}

static bool gmf_play_file(const char* uri) {
    if (!gmf_init_codec_chain(44100)) {
        return false;
    }
    if (esp_audio_simple_player_set_volume(g_gmf_player, g_audio_volume_pct) != ESP_OK) {
        Serial.println("esp-gmf: failed to set initial volume");
    }

    esp_err_t err = esp_audio_simple_player_play(g_gmf_player, uri);
    if (err != ESP_OK) {
        Serial.printf("esp-gmf: play failed: %s\n", esp_err_to_name(err));
        return false;
    }

    while (true) {
        esp_audio_state_t st = esp_audio_simple_player_get_state(g_gmf_player);
        if (st == ESP_AUDIO_STATE_STOPPED || st == ESP_AUDIO_STATE_ERROR) {
            break;
        }
        vTaskDelay(pdMS_TO_TICKS(10));
    }
...
/**
 * Play a WAV file from SD card using esp-gmf's simple player. Returns true if
 * playback succeeded.
 */
bool playWavFile(const String& wavPath) {
    ...
    // Build a POSIX-style path for esp-gmf simple player and a FatFs path to
    // verify the file exists before attempting playback.
    String fatfsPath = "0:";
    String posixPath = "/sdcard";
    ...
    // Ensure any prior tone stream is stopped before switching playback backends
    if (g_audio_running) {
        g_audio_running = false;
        delay(10);
    }

    String uri = "file://" + posixPath;
    if (gmf_play_file(uri.c_str())) {
        Serial.println("  Playback complete via esp-gmf simple player");
        Serial.println("========================================\n");
        return true;
    }

    Serial.println("  esp-gmf simple player failed to play file");
    Serial.println("========================================\n");
    return false;
}
